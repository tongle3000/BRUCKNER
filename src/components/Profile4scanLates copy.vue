...
const handleAdd = () => {
    console.log('9999999999999999999999', obj.defaultNum)
    return obj.space += 0.1;
}
const handleMinus = () => {
    console.log('9999999999999999999999', obj.defaultNum)
    return obj.space -= 0.1;
...
const slices = computed(() => {
    ...
    const firstY = 2.5 * (243 * 5 / 200) ; // 200 5   243 x (初始y站2.5格)
    ...
    const middleValue = (referenceValue - startValue) * (121.5* obj.space);
    ...
    for (let i = 0; i < length * 2; i++) {
        ...
        if(i < length) {
            ...
            const newValue = (endValue - data[i].value) * (121.5* obj.space);
...
<a-button @click="handleAdd">每次+0.1</a-button>
<a-button @click="handleMinus">每次+0.1</a-button>
<div class="profile">
